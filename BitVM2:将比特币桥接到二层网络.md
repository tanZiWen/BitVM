...
在本节中，我们将讨论我们工作的实际限制，诸如比特币轻客户端等具体设计组件的替代方法，以及可能的扩展。

### 8.1 实际限制
BitVM2 利用了若干技巧和变通方法，以现有的比特币脚本功能实现乐观验证，而不需要硬分叉。
- **通过签名委员会模拟Covenant**。BitVM2 的一个限制在于需要使用签名委员会来模拟covenant，如第4.2节所述。由于我们仅使用m-of-m多重签名，即没有阈值签名，这种签名者集合可以通过协议如MuSig扩展到100多名签名者，远远超出比特币的“普通”多重签名，但代价是增加了链下通信和协调的难度。虽然此设置的实现超出了本文的范围，但我们注意到这种设置已在更大规模上成功执行，例如以太坊的KZG可信设置仪式，参与者超过14万人，执行通过一个简单的网站。

- **大型非标准交易** 目前，BitVM2 中的 Assert 和 Disprove 交易超过了比特币的 400kB "标准性" 规则，这意味着未修改的比特币全节点不会中继这些交易。为了让这些交易被矿工处理，我们必须通过其他通信渠道，而不是比特币的P2P网络。某些矿池目前提供公共服务来处理有效但非标准的交易，但这并不是理想的解决方案。要减少交易大小，可以参考第8.4节讨论的优化方案。

- **固定存款金额** BitVM2 的设计仅支持固定的存款金额，这限制了用户的灵活性。用户必须在侧链系统上获得准确的包裹BTC数量才能启动peg-out。为了解决这一问题，可以将桥接存款分布到不同大小的 BitVM2 实例上（例如0.5 BTC到100 BTC）。在实践中，peg-in 和 peg-out 通常会由专业用户或服务提供商来执行，具体可以参考第8.5节的讨论。

- **运营者必须垫付BTC** BitVM2 的设计要求运营者在peg-out时从自己的余额中垫付BTC。这样做的好处是用户无需等待挑战期结束，可以立即收到他们的BTC。然而，这也给运营者带来了资本成本，因为他们需要确保自己有足够的BTC储备，或能及时获取BTC来执行peg-out操作。这一成本会体现在运营者的费用fO中，或者通过额外的激励措施（例如共享侧系统交易费用）来弥补。未来，比特币共识层的更新或许能提供更灵活的支出者身份验证机制。

- **经济上的轻客户端安全性** 如第7节所述，BitVM2 的 PowPV 轻客户端设计提供了经济安全性，但它仍然容易受到具备大量算力的非理性攻击者的攻击。这些攻击者可能拥有足够的资源来破坏桥接机制。

- **侧系统轻客户端的复杂性** 本文假设侧系统是比特币 roll-up 系统，即通过在比特币区块链上发布数据承诺来使用比特币作为共识机制。虽然 BitVM2 可以用于将BTC桥接到其他独立于比特币的区块链上，但具体的实现复杂性取决于每个区块链的共识协议。与比特币相关的共识协议（如合并挖矿或Stakechains）可能比完全独立的权益证明网络（如以太坊）更容易实现验证。

### 8.2 使用 Winternitz 签名替代 Lamport 签名  
Winternitz 签名 [2] 可以替代 Lamport 签名，从而减少大约 50% 的数据承诺大小。

### 8.3 其他比特币轻客户端方法
**使用常规链上承诺的轻客户端**  
实现安全轻客户端功能的直接方法是要求运营者定期使用 Lamport 签名提交他们知道的最新比特币区块，这允许挑战者提出挑战。然而，由于大量的通信开销和相关的链上成本，这种方法在实际操作中是不切实际的。

**使用乐观链上承诺**  
比上述设计稍微改进的一种方式是将承诺设计为乐观模式：运营者无需在链上提交最新的比特币区块数据，而是通过某种链外公告板与挑战者共享这些数据（公告板也可以是另一个数据存储费用较低的公共区块链）。如果出现数据丢失或争议，挑战者可以要求运营者进行链上提交，接着可以使用类似于第5.2节中描述的协议来发起挑战。

然而，在最坏的情况下，恶意挑战者可以通过持续发起挑战，强迫运营者将所有数据提交到链上。尽管可能的缓解措施是要求挑战者承担或分担运营者的链上成本，但对挑战者增加的资本要求使这一方案在实际操作中变得不切实际。

### 8.4 平衡 Assert 和 Disprove 交易的大小  
到目前为止，我们假设只有一个 Assert 交易，运营者必须在该交易中提交程序执行的中间状态。考虑到比特币区块大小的限制，我们估计可以通过单个 4MB 的 Assert 交易，将程序拆分为最多 960 个 4MB 的子程序。在理论上，可以同时使用 n 个 Assert 交易，这样可以 (i) 减小每个 Assert 交易的大小，或者 (ii) 保持 Assert 交易大小为 4MB，但减小子程序的大小，从而减小 Disprove 交易的大小。因此，在 Claim 交易中，运营者必须提交每个 n 个子程序组在各个 Assert 交易中分割后的计算状态。  
这种方法的显著缺点是，当通过签名者委员会模拟 Covenant 时，预签名的交易数量会呈指数增长。具体而言，我们必须预先签署 2n 个 Payout 交易，以确保诚实的运营者在被恶意挑战者挑战时能够恢复资金，无论他们必须揭示哪个 Assert 交易。

### 8.5 通过原子交换实现快速灵活的桥接  
考虑到固定的存款大小以及需要运行额外的软件来启动 BitVM2 存款，我们预计 BitVM Bridge 中的 peg-ins 和 peg-outs 将由专业用户作为一种服务来执行，可能由运营者自己作为做市商。我们预计大多数用户会使用跨链交换进入和退出侧链系统，类似于文献 [9] 中的描述。  
具体来说，在我们的 BitVM Bridge 协议中，Alice 不仅会进行 peg-in 操作，还会处理 peg-out 操作。为此，Alice 需要同时在比特币上的 B 和侧链系统上的 Bs 保持余额，向 Bob 提供交换服务并收取费用，同时通过执行 peg-in 和 peg-out 操作重新平衡资金。我们注意到这种模式已在以太坊和以太坊 L2 上通过所谓的 "流动性桥接" 大规模实施。

### 8.6 运营者轮换  
在我们的设计中，我们假设每个 BitVM2 实例都有预定义的运营者集。虽然可以在运行时添加或删除运营者，但这种机制的实现取决于 Covenant 模型。在我们的模型中，通过签名者委员会模拟 Covenant 时，m 个签名者无法在后续添加或移除运营者，因为这将违反我们假设至少有一个诚实签名者删除其密钥的协议和模型。  
即使这种修改是可能的，例如通过使用密钥演变密码学，但我们的签名者集为了安全起见设计得较大。因此，协调开销可能很大，失败率（例如签名者离线）可能较高，这使得对于现有的 BitVM2 实例进行临时的运营者轮换变得不切实际。相反，我们可以在 BitVM2 规则集中引入预定义的周期性轮换计划，即 SNARG 验证器。在这种模式下，只要每个 BitVM2 实例存在一个诚实的运营者，我们可以通过执行一个同时作为新的 BitVM2 实例的 peg-in 交易的 peg-out 交易来轮换现有实例的运营者集。类似的轮换设计已经在 tBTCv2 中实施。  
关于如何选择 BitVM2 实例的运营者的机制设计超出了本文的范围。例如，Proof-of-Stake 系统中实现的设计是基于随机抽样的质押代币，例如 BTC。同样，也可以根据在特定时间段内执行的工作量证明进行运营者的抽样，例如在与比特币合并挖矿的侧链系统中。

### 8.7 共识变更增强 BitVM2

有一系列提议的共识变更可以改善桥接设计，最显著的是大整数运算和 Covenants 的引入。

- **大整数运算**  
  正如 "Great Script Restoration" 提案中所建议的那样，引入大整数运算能够显著减少 SNARK 验证器的脚本大小，从 GB 级别缩减到 MB 级别，这可能允许在单个交易中完成 SNARK 验证，从而大幅简化整体设计。

- **Covenants 提案**  
  任何允许对交易输入进行承诺的 Covenants 提案都可以消除签名者委员会的需求，并保证存款的无条件安全。潜在的提案包括 introspection 操作码 (如 OP_INSPECTINPUTOUTPOINT)、TXHASH 或 OP_CAT。引入 introspection 操作码是实现这一目的最有效的方法，能够降低交易费用。

此外，借助 OP_CAT 操作码，SNARK 验证器可以被 STARK 验证器取代，从而避免为证明系统进行可信设置的需要。通过 OP_CAT，Assert 和 Disprove 交易的大小也可以被缩减，从而降低链上成本。最后，OP_CAT 还能够使得 OP_BLOCKHASH 的仿真更加简单和稳健，尽管理想情况下，OP_BLOCKHASH 应该得到原生支持。

这些共识级别的增强措施将为 BitVM2 提供更高效和安全的设计选项，并降低复杂性和成本，使桥接更容易实现和运行。
