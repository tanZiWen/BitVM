### 什么是可靠广播
可靠广播（Reliable Broadcast）是一种分布式系统中的通信机制，旨在确保消息从一个发送者（通常称为源节点）正确且一致地传递到所有其他节点，即使系统存在故障（如节点崩溃或消息丢失）。它在分布式计算、区块链、数据库复制等领域有广泛应用。
#### 可靠广播的核心属性
可靠广播通常需要满足以下几个关键属性：
1. 有效性（Validity）：如果一个正确的发送者广播了一条消息，那么所有正确的节点最终都会接收到这条消息。
2. 一致性（Agreement）：如果一个正确的节点接收到了一条消息，那么所有正确的节点最终都会接收到相同的消息。
3. 完整性（Integrity）：每条消息只会被接收一次，并且接收到的消息确实是由发送者发出的（防止伪造）。
4. 终止性（Termination）：所有正确的节点最终都会完成消息的接收（即算法会终止）。
#### 与普通广播的区别
普通广播可能只保证消息尽量送达，但不一定能处理故障情况。可靠广播则更强，它能够在部分节点故障（如崩溃或恶意行为，取决于具体模型）的情况下，仍然保证上述属性。
#### 实现方式
可靠广播的实现通常依赖于以下技术：
1. 确认机制（Acknowledgments）：节点收到消息后发送确认，发送者等待足够多的确认以确保消息被正确传播。
2. 重传机制：如果检测到消息丢失，系统会重新发送消息。
3. 一致性协议：如在拜占庭容错（Byzantine Fault Tolerance, BFT）场景中，使用类似PBFT（Practical Byzantine Fault Tolerance）协议来应对恶意节点。
4. 加密签名：确保消息的完整性和来源可验证。
